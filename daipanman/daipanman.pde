...
void draw()
{
  ...
  float diameter = abs(recvDatacoefficient);         //受信したデータから円の直径を計算
  stroke(white);
  ellipse(width/2, height/2, diameter, diameter);     //円を描画
  if (recvData>50) {
    try {
      Runtime r = Runtime.getRuntime();
      Process process = r.exec("processing-java --run --sketch=/Users/Paradise_on_earth/Documents/Processing/daipan_die  --output=出力先のパス --force");
    }
    catch (Exception e) {
      println(e);
    }
